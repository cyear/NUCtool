...
pub fn calculate_speed(points: &Vec<FanPoint>, temperature: u8) -> u8 {
    ...
    if temperature >= points[points.len() - 1].temperature {
        return points[points.len() - 1].speed.clamp(0, 100);
    }
    ...
    for window in points.windows(2) {
        ...
        if temperature >= p1.temperature && temperature <= p2.temperature {
            let temp_range = (p2.temperature - p1.temperature) as f32;

            let temp_offset = (temperature - p1.temperature) as f32;

            let ratio = temp_offset / temp_range;

            let speed1 = p1.speed as f32;

            let speed2 = p2.speed as f32;

            let speed = speed1 + (speed2 - speed1) * ratio;
